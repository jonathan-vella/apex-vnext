...
test("bootstrap reuses an exact local runtime and rejects a conflicting version", async () => {
  ...
  await writeFile(join(root, "node_modules", "@apexops", "cli", "package.json"), '{"version":"0.10.0-next.1"}\n');
...
test("CLI rejects a stale Terraform lock hash", async () => {
  ...
  assert.deepEqual(configured, { version: "0.10.0-next.1", bundleVersion: "0.10.0-next.1", configVersion: "1.0.0" });
});
