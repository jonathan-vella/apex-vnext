...
| `markdown-policy-enforcement`   | `.markdownlint-cli2.jsonc` owns executable markdownlint rules and exclusions; required CI and lefthook consume `lint:md`, while `docs.yml` owns links and freshness. | Keep one hosted owner per check while retaining audience-specific prose guidance.        | Direct, hook, editor, and hosted checks must preserve scope, diagnostics, exclusions, and nonzero tool failures. |
...
| Diagram routing                 | Mermaid owns inline diagrams and Python owns standalone diagrams; Draw.io is retired from active tooling.                                                            | Keep format-neutral semantics and negative reintroduction checks under `OWN-014`.        | `npm run validate:skills` and `npm run validate:diagram-semantics`; preserve frozen historical evidence.         |
...
| Cluster                         | Current owner and consumers                                                                                                                        | Disposition                                                                                                                                         | Later proof and gate                                                                                                           |
| ------------------------------- | -------------------------------------------------------------------------------------------------------------------------------------------------- | --------------------------------------------------------------------------------------------------------------------------------------------------- | ------------------------------------------------------------------------------------------------------------------------------ |
| `repository-validator-graph`    | `package.json` owns canonical command names; CI, pre-push checks, contributors, and release qualification consume them.                            | Keep focused commands where they provide a distinct workflow; retire proven-unused aliases and compose shared checks through `validate:_node-core`. | `npm run validate:_node-ci`; preserve recursive CI safety, parallel-safety constraints, diagnostics, and required check names. |
...
| `precommit-hooks`               | `lefthook.yml` owns staged-file selection and re-staging; npm scripts own validator behavior. Contributors consume hook diagnostics before commit. | Run focused checks in parallel; the SKU renderer is the only `stage_fixed` writer and structural tests enforce that boundary.                       | `npm run validate:hooks` and `npm run test:hooks`; preserve staged-file semantics and direct-command parity.                   |
...
| `exact-head-release-qualification`          | `release-candidate-qualification.yml` checks out the exact head without persisted credentials and uploads candidate-bound scorecard evidence. Required CI separately owns validators, tests, and clean-install packaging. | Keep the evidence-producing trust boundary separate, but do not repeat required CI commands. Run isolated release samples with bounded deterministic parallelism. | `npm run qualify:vnext-release`; preserve scorecard sample counts and ordering, checksums, artifacts, retention, exact-head binding, and denial of merge, publication, deployment, tag, or cutover authority. |
