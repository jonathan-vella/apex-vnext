...
Maintainer direction on 2026-09-24 amends DECISION-026 and DECISION-027. Governance discovery runs after Gate 1 and
before Architecture, so the Architect designs against known policy. It imports the reviewed collected subscription
baseline when one exists. Otherwise it imports a shipped ALZ Corp reference baseline, generated by a maintainer script
from a pinned Azure Landing Zones Library release (root, landing zones and corp archetypes) in the collector's finding
format. Local targets always use the reference. A subscription target on the reference can pass Gate 2, as in
pre-sales without Azure access, but its reviewed subscription baseline must replace the reference before Gate 3.
