...
| 5   | `apex-next` and sidekick      | New skill and sidekick agent, coordinator body                                                 | The coordinator routes through `apex-next`; delegation falls back to the client's selection step and a prompt; the sidekick only reads status and next task                                                                                             | Skill and agent validators, routing regression, CLI probe              |
