...
1. **Local writer to CI:** package the minimum repository-backed APEX run state and accepted writer-transfer claim into a
   deterministic envelope. Encrypt it with AES-256-GCM for the intended workflow recipient and upload it to a dedicated
   `handoff` Blob container using Microsoft Entra authentication. The blob name includes its digest; metadata binds the
   candidate commit, writer epoch, recipient, and expiry. The CI job downloads it after OIDC login, verifies and accepts
   the claim, imports state atomically, and deletes the blob after successful acceptance.
...
4. **Network boundary:** the backend firewall defaults to Deny. A job adds only its current runner `/32` after OIDC login
   and removes it in an unconditional cleanup step. The approved exception expires on `2026-07-16T12:50:34Z`.
...
- Uses `swedencentral` for all Azure resources; no allowed-location policy was discovered.
...
- Store handoff blobs in a dedicated container with versioning, soft delete, short lifecycle expiry, and no anonymous access.
