...
3. Aggregate into `review-index.json`:
  - `agents[]` — from `customizations/manifest.json` plus parsed
   ...
   - `docs_pages[]` — files under `docs/` with their inbound and outbound links.
...
1. Run `npm run validate:docs` to detect style, link, and freshness drift.
2. Confirm the index in `docs/README.md` covers the current guide and project trees.
3. Confirm count-driven pages read from
   `count-manifest.json` instead of hard-coded numbers.
4. Cross-check key claims against the review index and implementation.

Domain report: `08-docs.md`.
