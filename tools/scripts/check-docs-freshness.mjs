 ...
 *
 * Canonical documentation source: docs/
...
async function checkProhibitedRefs(cachedDocsFiles) {
  const prohibited = [
  ...
  // Scan repository docs and instructions
  ...
  const mdFiles = [...cachedDocsFiles].filter((f) => !relative(ROOT, f).toLowerCase().includes("changelog"));
...
async function checkSupersededLinks(cachedDocsFiles) {
  const deprecatedPaths = [/_superseded\//, /\.github\/templates\//];

  for (const file of cachedDocsFiles) {
    const content = await readText(file);
    if (!content) continue;
    const rel = relative(ROOT, file);
    const lines = content.split("\n");
...
async function checkVersionHeaders(cachedDocsFiles) {
...
async function main() {
  ...
  const docsFiles = await collectMdFiles(docsDir, ["docs/vnext/phase-0a/evidence"]);

  console.log("─── Prohibited References ───");
  await checkProhibitedRefs(docsFiles);

  console.log("─── Superseded Links ───");
  await checkSupersededLinks(docsFiles);

  console.log("─── Skill References Freshness ───");
  await checkSkillReferences();

  console.log("─── Version Header Check ───");
  await checkVersionHeaders(docsFiles);
